...
const Computing = ({ searchQuery, addToCart }) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (searchQuery) {
        ...
        response = await axiosInstance.get("/products/computing");
      }
  ...
  return (
    <div style={{ backgroundColor: "#f5f0ff", height: containerHeight }}>
      <div className="bg-warning mb-3">
        <h1 className="mt-5 text-center text-muted fw-bold p-3">Computing</h1>
      </div>
      <div className="container" style={{ backgroundColor: "white" }}>
        <div className="row">
          {products.map((product) => (
